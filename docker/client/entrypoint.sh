...
VITE_GIFBOX_URL="${VITE_GIFBOX_URL:-${REVOLT_PUBLIC_URL%%/api*}/gifbox}"
...
find /usr/share/nginx/html -name "*.js" -exec sed -i \
    ...
    -e "s|__VITE_GIFBOX_URL__|${VITE_GIFBOX_URL}|g" \
    -e "s|__VITE_HCAPTCHA_SITEKEY__|${VITE_HCAPTCHA_SITEKEY:-}|g" \
    -e "s|__VITE_CFG_ENABLE_VIDEO__|${VITE_CFG_ENABLE_VIDEO:-}|g" \
    {} +
